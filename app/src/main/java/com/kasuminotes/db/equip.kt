...
suspend fun AppDatabase.getEquipData(equipmentId: Int): EquipData {
...
suspend fun AppDatabase.isCraft(equipmentId: Int): Boolean {
...
private suspend fun AppDatabase.getEquipCraft(equipmentId: Int, consumeNum: Int, checked: Boolean): EquipCraft {
    return withIOContext {
         if (checked || isCraft(equipmentId)) {
            ...
            val conditions: List<EquipCraft> = use {
                rawQuery(sql, null).use {
                    it.moveToFirst()
                    var i = 0
            ...
            val conditionList: List<EquipCraft> = conditions.map { condition ->
